...
// .wrangler/tmp/bundle-0xvEkn/strip-cf-connecting-ip-header.js
...
var vs = I((Et) => {
  ...
  Et.Parser = void 0;
  var D = Yr(), sc = bs(), nn = 1, oc = 4, Ss = nn + oc, xs = y.allocUnsafe(0), on2 = /* @__PURE__ */ __name(class on {
    constructor(e) {
...
// src/lib/db.ts
function getConnString(env2) {
  const { PGHOST, PGDATABASE, PGUSER, PGPASSWORD } = env2;
  return `postgresql://${PGUSER}:${PGPASSWORD}@${PGHOST}/${PGDATABASE}?sslmode=require`;
}
__name(getConnString, "getConnString");
function getDb(connString) {
  return Ys(connString);
}
__name(getDb, "getDb");
...
__name(secretBytes, "secretBytes");
async function signJwt(payload, secret, expiresIn = "15m") {
  const [amount, unit] = [parseInt(expiresIn), expiresIn.replace(/\d+/g, "")];
  const expMap = { s: "seconds", m: "minutes", h: "hours", d: "days" };
  const jwt = await new SignJWT(payload).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setExpirationTime({ [expMap[unit] || "minutes"]: isNaN(amount) ? 15 : amount }).sign(secretBytes(secret));
  return jwt;
}
__name(signJwt, "signJwt");
...
auth.post("/login", async (c) => {
  ...
  if (!email || !password)
    return c.json({ error: "Email e senha obrigat\xF3rios" }, 400);
...
`;
  const user = rows[0];
  if (!user)
    return c.json({ error: "Credenciais inv\xE1lidas" }, 401);
...
var src_default = {
  fetch: (req, env2, ctx) => app.fetch(req, env2, ctx)
};
...
// .wrangler/tmp/bundle-0xvEkn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0xvEkn/middleware-loader.entry.ts
